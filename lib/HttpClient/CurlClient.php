...
class CurlClient implements ClientInterface
{
    ...
    public function request($method, $absUrl, $headers, $params)
    {
        ...
        if (is_callable($this->defaultOptions)) { // call defaultOptions callback, set options to return value
            ...
            if (!is_array($opts)) {
                throw new Error\PaylenseError("Non-array value returned by defaultOptions CurlClient callback");
            }
        ...
        if ($method == 'get') {
            ...
            throw new Error\PaylenseError("Unrecognized method $method");
        }
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Paylense ($url).  Please check your "
                 . "internet connection and try again.  If this problem persists, "
                 . "you should check Paylense's service status at "
                 . "https://status.paylense.com, or";
                break;
            case CURLE_SSL_CACERT:
            case CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Paylense's SSL certificate.  Please make sure "
                 . "that your network is not intercepting certificates.  "
                 . "(Try going to $url in your browser.)  "
                 . "If this problem persists,";
                break;
            default:
                $msg = "Unexpected error communicating with Paylense.  "
                 . "If this problem persists,";
        }
    ...
    {
        if ($numRetries >= Paylense::getMaxNetworkRetries()) {
            return false;
        }
    ...
    private function sleepTime($numRetries)
    {
        ...
        $sleepSeconds = min(
            Paylense::getInitialNetworkRetryDelay() * 1.0 * pow(2, $numRetries - 1),
            Paylense::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = max(Paylense::getInitialNetworkRetryDelay(), $sleepSeconds);
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which Paylense use.
