...
class CurlClientTest extends TestCase
{
    ...
    {
        $this->origMaxNetworkRetries = Paylense::getMaxNetworkRetries();
        $this->origMaxNetworkRetryDelay = Paylense::getMaxNetworkRetryDelay();
        $this->origInitialNetworkRetryDelay = Paylense::getInitialNetworkRetryDelay();
    }
    ...
    {
        $stripeReflector = new \ReflectionClass('\Paylense\Paylense');
        ...
        $curlClientReflector = new \ReflectionClass('Paylense\HttpClient\CurlClient');
    ...
    {
        Paylense::setMaxNetworkRetries($this->origMaxNetworkRetries);
    ...
    {
        $fakeRandomGenerator = $this->getMock('Paylense\Util\RandomGenetator', ['randFloat']);
    ...
    public function testDefaultOptions()
    {
        ...
        $this->setExpectedException('Paylense\Error\PaylenseError', "Non-array value returned by defaultOptions CurlClient callback");
    ...
    {
        Paylense::setMaxNetworkRetries(2);
    ...
    public function testShouldNotRetryAtMaximumCount()
    {
        ...
        $this->assertFalse($this->shouldRetryMethod->invoke($curlClient, 0, 0, Paylense::getMaxNetworkRetries()));
    }
    ...
    public function testSleepTimeShouldGrowExponentially()
    {
        ...
        $this->assertEquals(
            Paylense::getInitialNetworkRetryDelay() * 1,
            $this->sleepTimeMethod->invoke($curlClient, 1)
        );
        $this->assertEquals(
            Paylense::getInitialNetworkRetryDelay() * 2,
            $this->sleepTimeMethod->invoke($curlClient, 2)
        );
        $this->assertEquals(
            Paylense::getInitialNetworkRetryDelay() * 4,
            $this->sleepTimeMethod->invoke($curlClient, 3)
        );
        $this->assertEquals(
            Paylense::getInitialNetworkRetryDelay() * 8,
    ...
    public function testSleepTimeShouldAddSomeRandomness()
    {
        ...
        $baseValue = Paylense::getInitialNetworkRetryDelay() * (0.5 * (1 + $randomValue));

        // the initial value cannot be smaller than the base,
        // so the randomness is ignored
        $this->assertEquals(Paylense::getInitialNetworkRetryDelay(), $this->sleepTimeMethod->invoke($curlClient, 1));
